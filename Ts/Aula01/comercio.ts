...
function filtrarProdutoPorCategoria(
  ...
  categoria: string
): Produto[] {  //este :Produto[] define o tipo de retorno de toda a função
...
function aumentarPreco(mercadoriaLoja: Produto[], porcentagem: number): Produto[] {
    return mercadoriaLoja.map((produto) => {
        ...
        return { ...produto, preco: novoPrecoFormatado}; //retorna um novo objeto Produto, só q atualizada com o novo preço
    });
}  
